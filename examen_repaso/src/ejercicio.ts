...
function contarPalabras(){
    const palabra: string = (<HTMLInputElement>document.getElementById("valor")).value;
    ...
    for(let i = 0; i<palabra.length; i++){
        ...
        }else resultado += palabra.charAt(i).toLowerCase();
    }

    if(palabra.length%4==0){
        console.log("Es divisible "+ resultado);
    }else console.log("No es divisible " + resultado);
}