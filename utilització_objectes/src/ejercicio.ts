function diaDeLaSetmana(): void {
    // Recupera el valor de l'input
    const dataInput: string = (<HTMLInputElement>document.getElementById('data')).value;
    ...
    const dia: number = parseInt(parts[0], 10);6
    const mes: number = parseInt(parts[1], 10) - 1; // Els mesos van de 0 a 11
    const any: number = parseInt(parts[2], 10);

    // Crea un objecte Date
    const data: Date = new Date(any, mes, dia);

    // Obté el dia de la setmana (0 = diumenge, 1 = dilluns, ..., 6 = dissabte)
    const diaSetmana: number = data.getDay();

    // Mostra l'alert amb el dia de la setmana
    alert(diaSetmana);

    // Calcula l'arrel quadrada del dia de la setmana i ho mostra per consola
    const arrelQuadrada: number = Math.sqrt(diaSetmana);
    console.log(arrelQuadrada.toFixed(2));
}



function modificarBom(): void {
    // Recupera el valor de l'input
    const nombreInput: string = (<HTMLInputElement>document.getElementById('nombre')).value;

    // Converteix l'input a un número
    const nombreFinestres: number = parseInt(nombreInput, 10);
